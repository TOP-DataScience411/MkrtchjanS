...
def search_context(keyword, *keywords, n=0):
    """
    Функция для поиска контекста вокруг ключевых слов в текстовых файлах.
    """
    results = []
    keys = [keyword] + list(keywords)
    morph = pymorphy2.MorphAnalyzer()

    keyword_lemmas = {morph.parse(kw)[0].normal_form for kw in keys}
    ...
    for file in path.glob('*.txt'):
        try:
            with open(file, 'r', encoding='utf-8') as file_txt:
                lines = [remove_punctuation(line) for line in file_txt.readlines()]
        ...
        for line_num, line in enumerate(lines, start=1):
            # Лемматизация всех слов строки
            line_lemmas = {morph.parse(word)[0].normal_form for word in line.split()}
            
            # Проверка пересечения лемм строки с ключевыми леммами
            matching_lemmas = line_lemmas & keyword_lemmas
            if matching_lemmas:
